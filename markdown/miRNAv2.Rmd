...
```{r kaplan-meier, cache=TRUE, results='hide'}
...
Hier werden die die Patienten in 2 Gruppen geteilt, ne nachdem, ob eine gewisse miRNA editiert ist oder nicht. Hier werden verwendet:  
*hsa-miR-1260b
*hsa-miR-5100
...
for(i in 1:67)
{
  sortData[i, "m1260b"]<-if(is.na(editedData[1, i])) "unedited" else "edited"
  sortData[i, "m5100"]<-if(is.na(editedData[2, i])) "unedited" else "edited"
}

kaplanmeierDataTTFT<-survfit(Surv(TTFT,TTFT_censor) ~m1260b, data=sortData)
...
kaplanmeierDataPFS<-survfit(Surv(PFS_time_months, PFS_1_is_event) ~m1260b, data=sortData)
...
Analog wird mit  hsa-miR-5100 verfahren

```{r kaplan-meier 5100, echo=FALSE}
kaplanmeierDataTTFT<-survfit(Surv(TTFT,TTFT_censor) ~m5100, data=sortData)
...
kaplanmeierDataPFS<-survfit(Surv(PFS_time_months, PFS_1_is_event) ~m5100, data=sortData)
