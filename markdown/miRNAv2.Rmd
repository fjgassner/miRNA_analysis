...
pheatmap(editedData[,order(res.hc$cluster)], #durch den order Befehl können andere CLuster verfahren als von pheatmap verwendet werden
         cluster_rows = FALSE, 
         cluster_cols = FALSE, 
         filename = "~/Documents/miRNA_analysen/miRNA_editing/graphics/editingCluster.pdf", 
         width=10, 
         height = 6,
         border_color = NA)
dev.off()
...
write.table(editedData, "~/Documents/miRNA_analysen/miRNA_editing/editingDepth.tab", sep = "\t")
...
pheatmap(graphData[,sortData$PatientID[order(sortData$Sorting)]],
         ...
         breaks = breaksList,
         annotation_col = annotation, 
         filename = "~/Documents/miRNA_analysen/miRNA_editing/graphics/editedExpression.pdf",
         width=10,
         height = 6,
         border_color = NA)
dev.off()
...
graphDataDE<-graphDataDE[rowSums(graphDataDE[, 1:44] >= 1 | graphDataDE[, 1:44] <= -1) == 44, ]
...
# editing status
sortData$m5100<-t(editedData)[,1]
sortData$m6503<-t(editedData)[,2]
...
Analog wird mit hsa-miR6503-3p verfahren
