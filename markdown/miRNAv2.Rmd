...
```{r MSigDB ,cache=TRUE, message=FALSE, eval = FALSE}
...
```{r combineFiles, cache=TRUE, message=FALSE, warning=FALSE, error=FALSE}
...
#MAIN LOOP
####################################
filenames<-list.dirs("~/Documents/miRNA_analysen/miRNA_editing/data/", full.names = FALSE, recursive = FALSE)
...
for(i in 1:length(filenames))
{
  ...
  file<-read.delim(paste0("~/Documents/miRNA_analysen/miRNA_editing/data/", filenames[i], "/", filenames[i], ".editing_info.txt"), header=TRUE, sep="\t")
  ...
  tryCatch(
  {
    file<-read.delim(paste0("~/Documents/miRNA_analysen/miRNA_editing/data/", filenames[i], "/", filenames[i], ".crt_counts.txt"), header=TRUE, sep="\t")
...
sortData<-data.frame(matrix(c(rep("NAME", 53), rep("B-CELL", 53)), 
                            nrow = 53, 
                            ncol=2))
sortData[1:44, 1]<-conversion[, 1]
sortData[45:53, 1]<-colnames(editedData)[45:53]
...
pheatmap(editedData[,sortData$PatientID[order(sortData$Sorting)]], #durch den order Befehl können andere CLuster verfahren als von pheatmap verwendet werden
         cluster_rows = FALSE, 
         cluster_cols = FALSE, 
...
pheatmap(editedData[,sortData$PatientID[order(sortData$Sorting)]],
         cluster_rows = FALSE, 
         cluster_cols = FALSE,
         annotation_col = annotation)
```
